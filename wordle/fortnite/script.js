...
function createGameBoard() {
    ...
    for (let i = 0; i < NUMBER_OF_GUESSES; i++) {
        ...
        // This is the single small image for this row
        const smallImg = document.createElement('img');
        smallImg.src = 'image (1).jpg';
        smallImg.alt = 'Small Correct Answer';
        imageOverlay.appendChild(smallImg);

        mainBoxesContainer.appendChild(imageOverlay);
        row.appendChild(mainBoxesContainer);
...
function handleKeyDown(event) {
 ...
 function checkGuess() {
    ...
    if (guessString === secretWord) {
        ...
        const imageOverlay = currentRowDiv.querySelector('.image-overlay');


        if (imageOverlay) {

            extraMessage("night...", 3000);

            setTimeout(() => {
                extraMessage("hmm sounds familiar...", 3000);

                setTimeout(() => {
                    extraMessage("OHH I KNOWW !!..", 4000);
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            largeImageContainer.classList.add('zoomed');
                        }, 500); // 0.5s delay to start the large image fade-in


                        

                        setTimeout(() => {
                            window.location.href = ' fortnight.html';
                        }, 2000);
